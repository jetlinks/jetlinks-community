...
class ElasticSearchRowModeQueryOperations extends RowModeQueryOperationsBase {
    ...
    protected Flux<ThingPropertyDetail> queryEachProperty(@Nonnull String metric,
        ...
        if (!param.isPaging() || param.getPageSize() >= 1000 || !settings
            .getProperty()
            .isQueryPropertiesAggregation()) {
    ...
    protected Flux<AggregationData> doAggregation(String metric, AggregationRequest request, AggregationContext context) {
        ...
        if (properties.length == 1) {
            Aggregation aggregation = properties[0].getAgg();
            return AggregationQueryParam
                .of()
                .agg(getPropertyColumn(aggregation), properties[0].getAlias(), aggregation, properties[0].getDefaultValue())
                .as(param -> {
        ...
        return AggregationQueryParam
            ...
            .as(param -> {
                Arrays.stream(properties)
                      .forEach(agg -> {
                          Aggregation aggregation = agg.getAgg();
                          param.agg(getPropertyColumn(aggregation), "value_" + agg.getAlias(), aggregation, agg.getDefaultValue());
                      });
                return param;
            })
            .as(param -> {
                                             ...
                                             return propsGroup
                                                 ...
                                                 .map(agg -> {
                                                     ...
                                                     aliasProperty.forEach((alias, prp) -> {
                                                         if (prp.getAgg() == Aggregation.FIRST || prp.getAgg() == Aggregation.TOP || prp.getAgg() == Aggregation.LAST) {
    ...
    private static String getPropertyColumn(Aggregation aggregation) {
        if (aggregation.needNumberValue()) {
            return ThingsDataConstants.COLUMN_PROPERTY_NUMBER_VALUE;
        }
        return ThingsDataConstants.COLUMN_PROPERTY_VALUE;
    }
}
